...
def user_create(request):
    if request.method == 'POST':
        ...
        userdocs=data.get('userdocs')
        account_nm=data.get('account_nm')
        address = data.get('address')
        lat = data.get('lat')
        long = data.get('long')
        device_nm = data.get('devicename')
        devicetype = data.get('devicetype')
        location_data = data.get('location_data')   
        device_location= Point(location_data)
        dev_loc = f"SRID=4326;{device_location}"
        account_location = Point(float(lat),float(long))
        acc_loc = f"SRID=4326;{account_location}"
    
        if User.objects.filter(Mobno=Mobno).exists() or Device.objects.filter(location=dev_loc).exists() or Account.objects.filter(location=acc_loc).exists():
            return JsonResponse({'error': 'User, Account_location or Device_location already exists'}, status=400)
        token = get_token(request)
        user_instance = Registration.objects.get(Mobno=Mobno)
        category = AdminUser.objects.get(user_category=user_instance.user_category)
        user = User(Name=user_instance.Name,Email=user_instance.Email, Mobno=Mobno, password=password, Adhaar=user_instance.Adhaar, token=token, user_category=category,user_pic=userpic, user_docs= userdocs)
        user.save()
        param = {
                'host':'20.244.37.91',
                'database':'logindb',
        ...
        print("connected")
        cur = conn.cursor() 
        cur.execute('INSERT INTO public.myapp_user("Name", "Email", "Mobno", "password", "Adhaar", "token", "user_category") VALUES (%s, %s, %s, %s, %s, %s, %s);', (f'{user_instance.Name}', f'{user_instance.Email}', f'{user_instance.Mobno}', f'{password}', f'{user_instance.Adhaar}', f'{token}', f'{category}'))
        conn.commit()
        while True:
            user_instance1= User.objects.get(Mobno=Mobno)
            accountid=random.randrange(10000000000000,9999999999999999)
            if not Account.objects.filter(Account_id=accountid).exists():
                account = Account(account_name=account_nm, Account_id=accountid, area= address,location=acc_loc, user=user_instance1)
                account.save()
                break
            else:
                continue
        ...
        while True:
            device_id = random.randrange(10000000000000,999999999999999)
            if not Device.objects.filter(device_id=device_id).exists():
                device_type_instance = DeviceType.objects.get(Name=devicetype)
                device = Device(device_id=device_id, device_name=device_nm,account=account, device_type=device_type_instance, location=dev_loc)
                device.save()
                break
            else:
                continue
                
        return JsonResponse({'message': 'User, account, and device created successfully'})
        # except Exception as e:
        #     return JsonResponse({'error': str(e)}, status=500)
...
@csrf_exempt
def userside_account_view(request,Account_id):
    if request.method == 'GET':
        ...
        if devices:
            ...
            return JsonResponse({"accounts":account_details})

        return JsonResponse({"message": account_details})


@csrf_exempt
def userside_user_view(request,Mobno):
    if request.method == 'GET':
        try:
            user=User.objects.get(Mobno=Mobno)
            final_list=[]
            user_list=[
                user.Mobno,
                user.Email,
            ...
            return JsonResponse({"message":final_list})
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=500)
        ...
        try:
            ...
            while True:        
                ...
                if not Device.objects.filter(device_id=random_id).exists():
                    ...
                    # cur.close() 
                    # conn.commit() 
                    new_device = Device(device_name=device_nm, device_type=device_type_instance, account=account, device_id=random_id,location=location)
                    new_device.save()
...
@csrf_exempt
def user_pic_docs(request):
    if request.method == 'POST':
        try:
            data = request.POST
            user_mob = data.get('Mobno')
            user_pic = request.FILES.get('user_pic')
            user_docs = request.FILES.get('user_docs')
            user=User.objects.get(Mobno=user_mob)

            if user:
                if user_pic and not user_docs:
                    user.user_pic=user_pic
                    user.user_docs=user.user_docs
                    user.save()
                    return JsonResponse({"user_pic_url": user.user_pic.url})
                
                elif user_docs and not user_pic:
                    user.user_pic=user.user_pic
                    user.user_docs=user_docs
                    user.save()
                    return JsonResponse({"user_docs_url": user.user_docs.url})

                elif user_pic and user_docs: 
                    user.user_pic = user_pic
                    user.user_docs = user_docs
                    user.save()
                    return JsonResponse({"user_pic_url": user.user_pic.url,"user_docs_url": user.user_docs.url})
...
@csrf_exempt
def imageview(request, Mob):
    if request.method == 'GET':
        try:
            
            user = User.objects.get(Mobno=Mob)
            if user.user_pic:
                response = {
                    'image': user.user_pic.url
                }
                return JsonResponse(response)
            else:
                return JsonResponse({'message': 'Image not found for this user'})
...
def calender_create(request):
    if request.method=="POST":
        ...
        try:
            if Calender.objects.filter(Message_id=Message_id).exists():
                return JsonResponse({"error":"Message_id already exist"}) 
            else:
                user_instance = User.objects.get(Mobno=Mobno)
                new_calenderdata=Calender(Mobno=user_instance,date=date,Message_id=Message_id,Message=Message)
                new_calenderdata.save()
                return JsonResponse({"message":"Message created"},safe=False)
                
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=500)
        
@csrf_exempt
def calender_view(request,Mobno):
    if request.method=="GET":
        try:
            calender_data = Calender.objects.filter(Mobno=Mobno)
            print(calender_data)
            final_list=[]
            ...
            for i in calender_data:
                ...
                print(calender_list)
                final_list.append(calender_list)
            print(final_list)
            return JsonResponse({"message":final_list})
...
@csrf_exempt
def calender_delete(request):
    if request.method=="POST":
        data = JSONParser().parse(request)
        Mobno = data.get("Mobno")
        Message_id = data.get("Message_id")
        try:
            get_calenderdata=Calender.objects.get(Mobno=Mobno,Message_id=Message_id)
            get_calenderdata.delete()
...
@csrf_exempt
def userside_devicetype(request,account_id):
    try:
        if request.method == 'GET':
            device_list=Device.objects.filter(account=account_id)
            devicetype_list=[]
            for device in device_list:
                ...
                if devicetype not in devicetype_list:
                    devicetype_list.append(devicetype)
            return JsonResponse({'message': devicetype_list})
    except Exception as e:
            return JsonResponse({'error': str(e)}, status=500)
...
def param_update(request):
    if request.method == 'PATCH':
        data = JSONParser().parse(request)
        user_mob = data.get('Mobno')
        devicetype = data.get('device_type')
        param = data.get('param')
        if not DeviceType.objects.filter(Name=devicetype).exists():
            return JsonResponse({'error': 'DeviceType does not exists'}, status=400)
        else:
            try:
                user = User.objects.get(Mobno=user_mob)
                ...
                for account in accounts:
                    print(account)
                    devices = Device.objects.filter(account=account,device_type__Name=devicetype)
                    print(devices)
                    for device in devices:
                        print(device)
                        device.sensors.append(param)
                        device.save()
                return JsonResponse({"device":"sensor updated succesfully"})
                    ...
                    for device in devices:
                        ...
                        if sensor:
                            if param in sensor:
                                ...
                                return JsonResponse({'error': 'Param does not exist'}, status=400)
                        else:
                            return JsonResponse({'error': 'sensor does not exist'}, status=400)
                return JsonResponse({'message': 'Param deleted successfully'}, status=200)
            except Device.DoesNotExist:
                return JsonResponse({'error': 'Device does not exist'}, status=400)
            except Exception as e:
                return JsonResponse({'error': str(e)}, status=500)
...
def calender_delete(request):
    if request.method=="POST":
        ...
        Message_id = data.get("Message_id")
        try:
            get_calenderdata=Calender.objects.get(Message_id=Message_id)
            get_calenderdata.delete()
            return JsonResponse({"message":"Message deleted successfully"})
...
def thermal_actual_image(request,mobno,user_given_day):
    if request.method == 'GET':
        ...
        try:
            ...
            a = [f"http://4.188.244.11/media/{i.image.name}" for i in data]
...
@csrf_exempt
def userside_graph_view(request,Account_id):
    if request.method == 'GET':
        try:
            data_accnt = Account.objects.get(Account_id=Account_id)
            data = Device.objects.filter(account=data_accnt)
            final_data = {}
            for i in data:
                print(i)
...
@csrf_exempt
def userside_device_view(request,Account_id):
    if request.method == 'GET':
        try:
            data_accnt = Account.objects.get(Account_id=Account_id)
            print(data_accnt)
            ...
            for i in data:
                ...
                if coordinates:
                    # coordinates = coordinates.strip('POINT ()').split()
                    coord_str = coordinates.split('(')[-1].split(')')[0]
                    coors_parts = coord_str.split()
                    first_coordinate, second_coordinate = map(float, coors_parts)
                    loc = [first_coordinate, second_coordinate]
                    device_type = DeviceType.objects.filter(Name=i.device_type)
                    print(device_type)
                    for types in device_type:
                        # final_output = [i.device_name,i.device_id,loc,types.controls[0]['button']['virtual_pin']]
                        final_output = [i.device_name,i.device_id,loc]
...
def test(request):
    ...
    if client_ip:
        # try:
        try:
            location = geolocator.geocode(query=client_ip, timeout=100)
            print(location)
        except Exception as e:
    ...
    #         return JsonResponse({'error': 'Geocoding timed out.'}, status=500)
    #     except Exception as e:
    #         # Log other exceptions for debugging
    #         print(f"Exception occurred during geocoding: {e}")
    #         return JsonResponse({'error': 'An error occurred during geocoding.'}, status=500)
    
    # return JsonResponse({'error': 'Unable to fetch device location.'}, status=500)