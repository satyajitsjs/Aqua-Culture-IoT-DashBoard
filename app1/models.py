...
def user_img(instance,filename):
    current_dt = datetime.datetime.now()
    date_str = current_dt.strftime("%d-%m-%Y")
    return '/'.join(['user_profile',str((instance.Name)+" "+date_str),filename])
...
class Device(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.sensors:
            if self.device_type and  self.device_type.Name == 'Aeration':
    ...
    #         self.sensors = ['current', 'voltage']
    #     elif self.device_type.Name == 'Monitoring':
    #         self.sensors = ['do', 'ph', 'orp']
    #     else:
    #         self.sensors = []
...
class Calender(models.Model):
    Mobno = models.ForeignKey(User,on_delete=models.CASCADE)
    date = models.DateField(default=timezone.now)
    Message_id= models.CharField(primary_key=True)
    Message = models.TextField()
    def __str__(self):
        name=f"{self.Message_id}"
        return name
